...
// types
import { ContextInterface, SearchTrackType } from '../../../types/types';
...
const SearchTrackInfo: React.FC<TrackProps> = ({ track, index }) => {

    const { setCurrentTrack } = useAppContext() as ContextInterface; 
            ...
            <div className='py-3 px-2 text-left min-w-[35vw] max-w-[35vh] w-[35vw] md:min-w-[25vw] md:max-w-[25vh] md:w-[25vw] flex flex-grow md:flex-grow-0'>
                {track.album.images.length > 0 ? 
                    <div className='w-12 h-12 max-w-[3rem] min-w-[3rem] md:flex items-center justify-center hidden'>
                        <img width={40} height={40} src={track.album.images[2].url} alt={track.name} />
                    </div>
                    :
